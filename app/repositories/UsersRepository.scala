...
class UsersRepository @Inject()
    ...
    (implicit executionContext: ExecutionContext) extends UsersComponent with HasDatabaseConfigProvider[JdbcProfile] {
...
trait UsersComponent { self: HasDatabaseConfigProvider[JdbcProfile] =>
    ...
    /** Query for the USERS table, returning a copy of the row after a database operation */
    lazy protected val usersReturningRow = users returning users.map(_.id) into { (rota, id) =>
        rota.copy(id = id)
    }
